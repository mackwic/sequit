...
Pure builders, their deterministic IDs/seeds, topology registry and node-count matrix now live in `src/app/workshop/fixtures/layout-performance/`. Tests import them directly. Preparation with synthetic layout measurements stays in this directory. The `/atelier` document picker consumes the same builders with real browser rendering; it does not import this test harness.
