...
test.describe('AI for documentary effort', () => {
	...
	test('rejects a keyless source while preserving the persisted endpoint order', async ({
		...
		const sourceContract = await page.evaluate(async () => {
			...
			const sourceModule: unknown = await importModule('/src/app/web/examples/example-document.ts');
			const openDocumentModule: unknown = await importModule(
				'/src/app/web/projection/open-document.ts',
			);
			if (
	...
	test('renders the crossing-aware editing pipeline in a browser integration', async ({ page }) => {
		...
		await page.evaluate(async (source) => {
			...
			const canvasSessionModule = await importModule(
				'/src/app/web/ui/session/canvas-session.svelte.ts',
			);
		...
		const rendererContract = await page.evaluate(() => {
			...
			function routeContract(root: HTMLElement) {
				...
				return [...root.querySelectorAll<SVGPathElement>('[data-relation-id]')].map((path) => {
					...
					const commands = d.match(/[ALM][^ALM]*/g) ?? [];
					const points = commands.map((command) => {
						const coordinates = (command.match(/-?\d+(?:\.\d+)?(?:e[+-]?\d+)?/gi) ?? []).map(
							Number,
						);
						const x = coordinates.at(-2);
						const y = coordinates.at(-1);
						if (x === undefined || y === undefined) throw new Error('Incomplete route command');
						return { x, y };
					});
